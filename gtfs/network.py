...
def link_trips(trip_dicts, services_by_id, shapes_by_id):
    ...
    for trip_dict in trip_dicts:
        ...
        shape = shapes_by_id.get(trip_dict["shape_id"])
    ...
    for stop_dict in stop_dicts:
        ...
        if stop_dict["location_type"] == LocationType.STOP:
            stop = Stop(
                ...
                **get_station_stop_args_from_dict(stop_dict)
            )
            stops.append(stop)
            if parent_station:
        ...
        for route_pattern_dict in matching_route_patterns:
            route.route_patterns.append(
                RoutePattern(
                    ...
                    name=route_pattern_dict["route_pattern_name"],
                    route=route,
...
def link_feed_info(feed_info_dicts):
    only_info_dict = feed_info_dicts[0]
    return FeedInfo(
        start_date=date_from_string(only_info_dict["feed_start_date"]),
        end_date=date_from_string(only_info_dict["feed_end_date"]),
        version=only_info_dict["feed_version"],
    )
...
def build_network_from_gtfs(loader: GtfsLoader):
    # Do the loading...
    feed_info = loader.load_feed_info()
    ...
    services_by_id = link_services(
        calendar_dicts, calendar_attribute_dicts, calendar_date_dicts
    )
    ...
    return Network(
        feed_info=link_feed_info(feed_info),
